                           ...
                           char  *ref_idx)     //!< reference pictures for each list
{
  ...
  int    block_y, block_x,bx,by, pic_pix_x, nonzero = 0, cbp_blk_mask;
  int    coeff_cost = 0;
  int    mb_y       = (block8x8 >> 1) << 3;
  int    mb_x       = (block8x8 & 0x01) << 3;
  int    cbp_mask   = 1 << block8x8;  
 ...
 all_mv = &currSlice->all_mv[0][0][0][0];
  ...
  if(!currMB->luma_transform_size_8x8_flag)
  {
    ...
    if (!direct_inference && (((p_dir == 0 || p_dir == 2 )&& list_mode[0] < 5) || ((p_dir == 1 || p_dir == 2 ) && list_mode[1] < 5)))
    {
      ...
      if(currSlice->slice_type == P_SLICE)
      	{
		  compute_residue_p8x8 (currMB,block8x8,0,&p_Vid->pCurImg[currMB->opix_y + mb_y], &currSlice->mb_pred[0][mb_y], &currSlice->mb_ores[0][mb_y], mb_x, currMB->pix_x + mb_x,list_mode, 8, 8);
      	}
	  else 
	  	{
    ...
    for (byy=0, block_y=mb_y; block_y<mb_y+8; byy+=4, block_y+=4)
    {
      for (bxx=0, block_x=mb_x; block_x<mb_x+8; bxx+=4, block_x+=4)
      {
        ...
        if (direct_inference || !(((p_dir == 0 || p_dir == 2 )&& list_mode[0] < 5) || ((p_dir == 1 || p_dir == 2 ) && list_mode[1] < 5)))
        {
          ...
          if(currSlice->slice_type == P_SLICE)
	      	{
				compute_residue_p4x4(currMB,block8x8,0,bx>>2, by>>2,&p_Vid->pCurImg[currMB->opix_y + block_y], &currSlice->mb_pred[0][block_y], &currSlice->mb_ores[0][block_y], block_x, pic_pix_x, list_mode,4, 4);
	        }
		  else 
		  	{
  ...
  else
  {
	...
	if(currSlice->slice_type == P_SLICE)
	{
		compute_residue_p8x8(currMB,block8x8,0,&p_Vid->pCurImg[currMB->opix_y + block_y], &currSlice->mb_pred[0][block_y], &currSlice->mb_ores[0][block_y], block_x, pic_pix_x,list_mode, 8, 8);
	}
	else 
	{ 
  ...
  for (uv = 0; uv < 2; ++uv)
  {
	...
	if(currSlice->slice_type == I_SLICE)
	{
	    ...
	    else
	    {
	      ...
	      chroma_cbp = currMB->residual_transform_quant_chroma_4x4[uv] (currMB, uv, chroma_cbp,list_mode);
	    }
	 }
	else if(currSlice->slice_type == P_SLICE)
	{
                          ...
                          )
{
  ...
  for (j = j0; j < j1; j += step_v)
  {
    ...
    for (i = i0; i < i1; i += step_h)
    {
      ...
      mvd[2] = cur_mv->scale[0]- predMV.scale[0];
      mvd[3] = cur_mv->offset[0]- predMV.offset[0];
	  mvd[4] = cur_mv->scale[1]- predMV.scale[1];
	  mvd[5] = cur_mv->offset[1]- predMV.offset[1];
	  mvd[6] = cur_mv->scale[2]- predMV.scale[2];
	  mvd[7] = cur_mv->offset[2]- predMV.offset[2];
