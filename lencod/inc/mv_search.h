...
static inline void add_mvs(MotionVector *mv0, const MotionVector *mv1)
{
  ...
  mv0->scale[0]= (short) (mv0->scale[0]+ mv1->scale[0]);
  mv0->offset[0]= (short) (mv0->offset[0]+ mv1->offset[0]);
  mv0->scale[1]= (short) (mv0->scale[1]+ mv1->scale[1]);
  mv0->offset[1]= (short) (mv0->offset[1]+ mv1->offset[1]);
  mv0->scale[2]= (short) (mv0->scale[2]+ mv1->scale[2]);
  mv0->offset[2]= (short) (mv0->offset[2]+ mv1->offset[2]);
}
...
static inline distblk mv_cost(const VideoParameters *p_Vid, int lambda, const MotionVector *mv, const MotionVector *pmv)
{
  ...
  return ( ((distblk)lambda) *((distblk)(mvbits[mv->mv_x - pmv->mv_x] + mvbits[mv->mv_y - pmv->mv_y]+ mvbits[mv->scale[0]- pmv->scale[0]]+ mvbits[mv->offset[0]- pmv->offset[0]]+ mvbits[mv->scale[1]- pmv->scale[1]]+ mvbits[mv->offset[1]- pmv->offset[1]]+ mvbits[mv->scale[2]- pmv->scale[2]]+ mvbits[mv->offset[2]- pmv->offset[2]])) );
#else
#if (USE_RND_COST)
  return (rshift_rnd_sf((lambda *(p_Vid->mvbits[mv->mv_x - pmv->mv_x] + p_Vid->mvbits[mv->mv_y - pmv->mv_y]+ p_Vid->mvbits[mv->scale[0]- pmv->scale[0]]+ p_Vid->mvbits[mv->offset[0]- pmv->offset[0]]+ p_Vid->mvbits[mv->scale[1]- pmv->scale[1]]+ p_Vid->mvbits[mv->offset[1]- pmv->offset[1]]+ p_Vid->mvbits[mv->scale[2]- pmv->scale[2]]+ p_Vid->mvbits[mv->offset[2]- pmv->offset[2]])), LAMBDA_ACCURACY_BITS));
#else
  return ((lambda *(p_Vid->mvbits[mv->mv_x - pmv->mv_x] + p_Vid->mvbits[mv->mv_y - pmv->mv_y]+ p_Vid->mvbits[mv->scale[0]- pmv->scale[0]]+ p_Vid->mvbits[mv->offset[0]- pmv->offset[0]]+ p_Vid->mvbits[mv->scale[1]- pmv->scale[1]]+ p_Vid->mvbits[mv->offset[1]- pmv->offset[1]]+ p_Vid->mvbits[mv->scale[2]- pmv->scale[2]]+ p_Vid->mvbits[mv->offset[2]- pmv->offset[2]]))>> LAMBDA_ACCURACY_BITS);
#endif
#endif
