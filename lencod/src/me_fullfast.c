...
void initialize_fast_full_search (VideoParameters *p_Vid, InputParameters *p_Inp)
{
  ...
  if ((p_me_ffast->search_center = (MotionVector**) malloc (8 * sizeof(MotionVector*)))==NULL)
...
void setup_fast_full_search (Macroblock *currMB, MEBlock *mv_block, int list)  // <--  reference frame parameter, list0 or 1
{
  ...
  p_Vid->p_ffast_me->search_center[list][ref].scale[0]= ((pmv.scale[0]+ 2) >> 2) * 4;
  p_Vid->p_ffast_me->search_center[list][ref].offset[0]= ((pmv.offset[0]+ 2) >> 2) * 4;
  
  p_Vid->p_ffast_me->search_center[list][ref].scale[1]= ((pmv.scale[1]+ 2) >> 2) * 4;
  p_Vid->p_ffast_me->search_center[list][ref].offset[1]= ((pmv.offset[1]+ 2) >> 2) * 4;
  
  p_Vid->p_ffast_me->search_center[list][ref].scale[2]= ((pmv.scale[2]+ 2) >> 2) * 4;
  p_Vid->p_ffast_me->search_center[list][ref].offset[2]= ((pmv.offset[2]+ 2) >> 2) * 4;
#else
  p_Vid->p_ffast_me->search_center[list][ref].mv_x = (pmv.mv_x / 4) * 4;
  p_Vid->p_ffast_me->search_center[list][ref].mv_y = (pmv.mv_y / 4) * 4;
  p_Vid->p_ffast_me->search_center[list][ref].scale[0]= (pmv.scale[0]/ 4) * 4;
  p_Vid->p_ffast_me->search_center[list][ref].offset[0]= (pmv.offset[0]/ 4) * 4;
  
  p_Vid->p_ffast_me->search_center[list][ref].scale[1]= (pmv.scale[1]/ 4) * 4;
  p_Vid->p_ffast_me->search_center[list][ref].offset[1]= (pmv.offset[1]/ 4) * 4;

p_Vid->p_ffast_me->search_center[list][ref].scale[2]= (pmv.scale[2]/ 4) * 4;
p_Vid->p_ffast_me->search_center[list][ref].offset[2]= (pmv.offset[2]/ 4) * 4;
#endif
  if (!p_Inp->rdopt)
  {
