  ...
  */
  ...
  case MVPRED_MEDIAN:
    ...
    else
    {
	  ...
	  pmv->scale[0]= (short) imedian(mv_a->scale[0], mv_b->scale[0], mv_c->scale[0]);
	  pmv->offset[0]= (short) imedian(mv_a->offset[0], mv_b->offset[0], mv_c->offset[0]);
      pmv->scale[1]= (short) imedian(mv_a->scale[1], mv_b->scale[1], mv_c->scale[1]);
	  pmv->offset[1]= (short) imedian(mv_a->offset[1], mv_b->offset[1], mv_c->offset[1]);	  
	  pmv->scale[2]= (short) imedian(mv_a->scale[2], mv_b->scale[2], mv_c->scale[2]);
	  pmv->offset[2]= (short) imedian(mv_a->offset[2], mv_b->offset[2], mv_c->offset[2]);
	  }    
    break;
  case MVPRED_L:
    if (block[0].available)
